...
export function runMigrations() {
    ...
    `);
        ...
        const masterKeyRecord = db.prepare("SELECT value FROM system_config WHERE key = 'master_key'").get();
        if (!masterKeyRecord) {
            const randomMasterKey = `sk_master_${crypto.randomBytes(16).toString('hex')}`;
            db.run("INSERT INTO system_config (key, value) VALUES ('master_key', ?)", [randomMasterKey]);
            console.log(`\n================================================================`);
        ...
        if (countResult && countResult.count === 0) {
            ...
            const activeMasterKey = db.prepare("SELECT value FROM system_config WHERE key = 'master_key'").get();
            const defaultKey = process.env.API_KEY || `sk_client_${crypto.randomBytes(16).toString('hex')}`;
            db.run("INSERT INTO api_keys (key_value, name, status, bucket_name, created_at) VALUES (?, ?, ?, 'default', ?)", [defaultKey, 'Default System Key (Dibuat Otomatis)', 'active', Date.now()]);
 ...
 */
export function getValidMasterKey() {
    ...
    try {
        const record = db.prepare("SELECT value FROM system_config WHERE key = 'master_key'").get();
        if (record) {
            return record.value;
        }
    }
