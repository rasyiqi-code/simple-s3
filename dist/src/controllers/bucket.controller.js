 ...
 */
export async function listBuckets(req, res, next) {
    try {
...
export async function createBucket(req, res, next) {
    ...
    try {
        ...
        // Simpan ke database
        db.run('INSERT INTO buckets (name, description, created_at) VALUES (?, ?, ?)', [bucketName, description.trim(), createdAt]);
        res.status(201).json({
            success: true,
 ...
 */
export async function deleteBucket(req, res, next) {
