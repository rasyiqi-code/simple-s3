...
export async function uploadSingleFile(req, res, next) {
    ...
    try {
        ...
        db.run('INSERT INTO files (original_name, filename, mime_type, size, uploaded_at) VALUES (?, ?, ?, ?, ?)', [originalName, finalFileName, finalMimeType, finalSize, Date.now()]);
        // Catat ke log audit aktivitas
        logActivity(apiKeyName, 'UPLOAD_SINGLE', `Unggah berkas: ${originalName} (Disimpan sebagai: ${finalFileName}, Ukuran: ${finalSize} bytes)`);
...
export async function uploadMultipleFiles(req, res, next) {
    ...
    try {
        ...
        // Catat ke log audit aktivitas
        logActivity(apiKeyName, 'UPLOAD_MULTIPLE', `Unggah ${files.length} berkas secara paralel. Berkas: ${processedFiles.map(f => f.originalName).join(', ')}`);
        res.status(201).json({
            success: true,
