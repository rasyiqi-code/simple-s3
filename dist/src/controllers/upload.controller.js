 ...
 */
function resolveBucket(req, res) {
...
export async function uploadSingleFile(req, res, next) {
    ...
    const apiKeyName = req.apiKeyName || 'Master Key / Admin';
    try {
        ...
        db.run('INSERT INTO files (original_name, filename, mime_type, size, bucket_name, uploaded_at) VALUES (?, ?, ?, ?, ?, ?)', [originalName, finalFileName, finalMimeType, finalSize, bucketName, Date.now()]);
        logActivity(apiKeyName, 'UPLOAD_SINGLE', `Bucket: ${bucketName} | Unggah berkas: ${originalName} (Disimpan sebagai: ${finalFileName}, Ukuran: ${finalSize} bytes)`);
...
export async function uploadMultipleFiles(req, res, next) {
    ...
    try {
        ...
        logActivity(apiKeyName, 'UPLOAD_MULTIPLE', `Bucket: ${bucketName} | Unggah ${files.length} berkas secara paralel. Berkas: ${processedFiles.map(f => f.originalName).join(', ')}`);
        res.status(201).json({
            success: true,
