...
function showKeys() {
  ...
  try {
    ...
    const masterRecord = masterQuery.get() as { value: string } | undefined;
  ...
  try {
    ...
    const clientRecords = clientQuery.all() as Array<{ key_value: string; name: string; status: string; bucket_name: string }>;
