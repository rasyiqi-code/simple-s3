 ...
 */
export async function deleteFileByClient(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
  const { filename } = req.params;
  const apiKeyName = (req as any).apiKeyName || 'Client Key';
  ...
  try {
    ...
    const fileRecord = db.prepare('SELECT * FROM files WHERE filename = ?').get(filename) as any;
    if (!fileRecord) {
      res.status(404).json({
        success: false,
    ...
    logActivity(
      apiKeyName,
      'DELETE_FILE',
      `Menghapus berkas: ${filename} (Nama Asli: ${fileRecord.original_name})`
    );
